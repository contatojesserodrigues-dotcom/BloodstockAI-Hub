...
// ============================================================
// BloodstockAI -- Training Analysis PDF Report Generator
// Premium layout mirroring the Catalog Lot report (cover, header/
// footer, gold dividers, score bars, sections).
// ============================================================
...
export async function generateTrainingReportPDF(horse: any, analyses: any[], sessions: any[] = []): Promise<jsPDF> {
  ...
  if (logoBase64) { try { doc.addImage(logoBase64, "PNG", PAGE_W / 2 - 15, 32, 30, 30); } catch {} }
  drawRect(doc, MARGIN, 72, CONTENT_W, 0.8, GOLD);

  doc.setFontSize(10); doc.setTextColor(GOLD[0], GOLD[1], GOLD[2]);
  doc.text("TRAINING PERFORMANCE REPORT", PAGE_W / 2, 84, { align: "center" });

  doc.setFontSize(22); doc.setTextColor(DARK_TEXT[0], DARK_TEXT[1], DARK_TEXT[2]); doc.setFont("helvetica", "bold");
  doc.text(sanitize(horse?.name || "Unnamed Horse"), PAGE_W / 2, 100, { align: "center" });
  doc.setFont("helvetica", "normal");

  const subtitle = [horse?.age ? `${horse.age}yo` : null, horse?.sex, horse?.breed].filter(Boolean).join(" | ");
  if (subtitle) {
    doc.setFontSize(10); doc.setTextColor(MID_GRAY[0], MID_GRAY[1], MID_GRAY[2]);
    doc.text(subtitle, PAGE_W / 2, 110, { align: "center" });
  }

  doc.setFontSize(9); doc.setTextColor(MID_GRAY[0], MID_GRAY[1], MID_GRAY[2]);
  doc.text(`${analyses.length} session${analyses.length === 1 ? "" : "s"} analysed`, PAGE_W / 2, 122, { align: "center" });
  ...
  // Average Scores
  y = sectionTitle(doc, "Average Performance Scores", y);
  for (const [k, label] of SCORE_KEYS) {
    y = checkPage(doc, y, 10);
    y = scoreBar(doc, label, avgScores[k] ?? 0, MARGIN, y);
  }
  y += 2;
